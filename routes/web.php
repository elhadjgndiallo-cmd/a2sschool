...
Route::get('/api/emploi-temps/{classeId}', function($classeId) {
    try {
        ...
        \Log::error('Erreur dans la route alternative emploi du temps: ' . $e->getMessage());
...
Route::get('/debug-emploi-temps-permissions', function() {
    ...
    return response()->json([
        ...
        'has_emplois_temps_view' => $user->hasPermission('emplois-temps.view'),
        'has_emplois_temps_view_old' => $user->hasPermission('emplois_temps.view'),
        'has_emploi_temps_view' => $user->hasPermission('emploi_temps.view'),
...
// Route de test simple pour vérifier le format de données
Route::get('/test-emploi-temps-format/{classe}', function($classeId) {
    $classe = App\Models\Classe::find($classeId);
    if (!$classe) {
        return response()->json(['error' => 'Classe non trouvée'], 404);
    }
...
Route::post('/test-delete-emploi-temps/{id}', function($id) {
    ...
    Route::get('/emplois-temps/classe/{classe}/data', function(App\Models\Classe $classe) {
        try {
            ...
            \Log::error('Erreur dans la route principale emploi du temps: ' . $e->getMessage());
            return response()->json(['error' => 'Erreur serveur: ' . $e->getMessage()], 500);
        }
    })->middleware('auth');
    ...
    Route::middleware('role:admin,personnel_admin')->group(function () {
        ...
        Route::post('/factures/preview-totaux', [\App\Http\Controllers\FacturationController::class, 'previewTotaux'])->name('factures.preview-totaux');
