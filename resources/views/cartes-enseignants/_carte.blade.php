@php
    $carte = $carte ?? $cartes_enseignant;
    $enseignant = $carte->enseignant;
    $user = $enseignant->utilisateur ?? null;
    ...
    $nom = trim($user->nom ?? '');
    $nomComplet = trim($prenom . ' ' . $nom) ?: ($user->name ?? 'Enseignant');
    $initiales = strtoupper(mb_substr($prenom !== '' ? $prenom : $nom, 0, 1) . mb_substr($nom, 0, 1));

    $idEnseignant = $enseignant->numero_employe ?? $carte->numero_carte;
    $specialite = $enseignant->specialite ?: 'Non renseignée';
    $email = $user->email ?? 'Non renseigné';
    $telephone = $user->telephone ?? 'Non renseigné';
    $adresseEnseignant = trim((string) ($user->adresse ?? '')) ?: 'Non renseignée';
        ...
        <div class="carte-ens-main">
            <div class="carte-ens-head">
                <div class="carte-ens-identity">
                    ...
                    <p class="carte-ens-job">Enseignant</p>
                </div>
            ...
            <div class="carte-ens-rows">
                <div class="carte-ens-row">
                    ...
                    <span>ID Enseignant : <strong>{{ $idEnseignant }}</strong></span>
                </div>
                <div class="carte-ens-row">
                    <i class="fas fa-book-open"></i>
                    <span>Spécialité : <strong>{{ $specialite }}</strong></span>
                </div>
                <div class="carte-ens-row carte-ens-row-wrap">
                    <i class="fas fa-map-marker-alt"></i>
                    <span>Adresse : <strong>{{ $adresseEnseignant }}</strong></span>
                </div>
                <div class="carte-ens-row">
