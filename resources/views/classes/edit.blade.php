        ...
        <form action="{{ route('classes.update', $classe) }}" method="POST">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('niveau') is-invalid @enderror" id="niveau" name="niveau" required>
                            ...
                            @foreach($niveaux ?? ['Préscolaire', 'Primaire', 'Collège', 'Lycée', 'Supérieur'] as $niveauOption)
                            <option value="{{ $niveauOption }}" {{ old('niveau', $classe->niveau) == $niveauOption ? 'selected' : '' }}>{{ $niveauOption }}</option>
                            @endforeach
                        </select>
                        @error('niveau')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('section') is-invalid @enderror" id="section" name="section" value="{{ old('section', $classe->section) }}" required>
                        @error('section')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('effectif_max') is-invalid @enderror" id="effectif_max" name="effectif_max" value="{{ old('effectif_max', $classe->effectif_max) }}" min="1" required>
                        @error('effectif_max')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="actif" name="actif" value="1" {{ old('actif', $classe->actif) ? 'checked' : '' }}>
