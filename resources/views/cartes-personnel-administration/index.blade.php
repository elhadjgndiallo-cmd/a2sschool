...
@section('title', 'Gestion des Cartes Personnel Administration')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2><i class="fas fa-id-card me-2"></i>Gestion des Cartes Personnel Administration</h2>
                <a href="{{ route('cartes-personnel-administration.create') }}" class="btn btn-primary">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Liste des Cartes Personnel Administration ({{ $cartes->total() }} cartes)</h5>
                </div>
                <div class="card-body">
                    @if($cartes->count() > 0)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    @if($carte->personnelAdministration->utilisateur->photo_profil && Storage::disk('public')->exists($carte->personnelAdministration->utilisateur->photo_profil))
                                                        <img src="{{ asset('storage/' . $carte->personnelAdministration->utilisateur->photo_profil) }}" 
                                                             alt="Photo personnel" 
                                                             class="rounded-circle me-2" 
                                                             style="width: 40px; height: 40px; object-fit: cover;">
                                                    @else
                                                        <div class="avatar-sm bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-2">
                                                            {{ substr($carte->personnelAdministration->utilisateur->nom, 0, 1) }}
                                                        </div>
                                                    @endif
                                                    <div>
                                                        <div class="fw-bold">{{ $carte->personnelAdministration->utilisateur->nom }} {{ $carte->personnelAdministration->utilisateur->prenom }}</div>
                                                        <small class="text-muted">{{ $carte->personnelAdministration->poste ?? 'Non renseigné' }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                    ...
                    @else
                        <div class="text-center py-5">
                            ...
                            <h5 class="text-muted">Aucune carte personnel administration trouvée</h5>
                            <p class="text-muted">Commencez par créer une nouvelle carte personnel administration.</p>
                            <a href="{{ route('cartes-personnel-administration.create') }}" class="btn btn-primary">
